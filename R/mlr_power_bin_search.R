...
mlr_power_bin_search <- function(
  ...
  first_left = left
  first_right = right
  search_range = first_right - first_left
  ...
  while (left <= right) {
    ...
    if(verbose == TRUE){
      cat(paste(rep(" ",times = length(number_line) * ((middle - first_left)/search_range)), collapse = ""),
        "N = ", middle,
        paste(rep(" ", times = length(number_line) * (1 - ((middle - first_left)/search_range))),collapse = ""),"\r",sep = "")
      }
